...
func mainHandler(w http.ResponseWriter, r *http.Request) {
	...
	// get list of orders : GetOrders(CustomerID)
	ordersList, err := GetOrders(0)
...
func customerHandler(w http.ResponseWriter, r *http.Request) {
	...
	// get customer object from customer API
	...
	orders, err := GetOrders(custID) // get orders list for this customer
	...
	var b strings.Builder
	b.WriteString(fmt.Sprintf("<h2>%s %s</h2><b>Lifetime Value: $%.2f</b> | <a href='javascript:history.back()'>back</a><br /><br /><table border='1'><thead><tr><th>ORDER ID</th><th>PLACED</th><th>STATUS</th><th>TOTAL</th></tr></thead><tbody>", thisCust.CustFN, thisCust.CustLN, custLifeTimeVal))
...
// Get orders list (for a given customer if a >0 customer ID is provided, then return orders list filtered by customer)
func GetOrders(customerID int) ([]Order, error) {
	...
	for err == nil && apiDataResp.StatusCode == http.StatusOK {
		...
		if err == nil && apiDataResp.StatusCode == http.StatusOK {
			...
			// request to get order data unsuccessful - return order data so far
			// fmt.Fprintf(os.Stderr, "Error fetching data from API endpoint (%s): %v\n", ordersAPILocation, err)
			// return orderList.OrdersList, errors.New(fmt.Sprintf("Error fetching data from API endpoint (%s): %v\n", ordersAPILocation, err))
			return fullOrderList, nil
		}
